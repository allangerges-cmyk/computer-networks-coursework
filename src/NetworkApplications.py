...
class Traceroute(ICMPPing):
    ...
    def sendIcmpProbesAndCollectResponses(self, ttl):
        ...
        for _ in range(3):
            ...
            timeSent = self.sendOnePing(self.dstAddress, packetID, seq_num, ttl, numBytes)
            ...
            inner_id, inner_seq, icmpType_outer = self.parseICMPTracerouteResponse(trReplyPacket)

            if inner_id == packetID and inner_seq == seq_num:
                rtts[seq_num] = timeRecvd - timeSent
                hop_addrs[seq_num] = hopAddr

                # Check if destination reached (Echo Reply = type 0)
                if icmpType_outer == 0:
    ...
    def parseICMPTracerouteResponse(self, trReplyPacket):
        ...
        #     Packet Identifier |       Sequence num   |
        # This header contains type, Code and Checksum + 4 bytes of padding (0's)
        # We only care about type field, outer id, and sequence number
        icmpType, _, _, inner_id, inner_seq = struct.unpack("!BBHHH", trReplyPacket[ip_header_len:ip_header_len + 8])
        ...
        if icmpType == 3 or icmpType == 11:
            ...
            # Extract the inner id and the inner sequence number (icmp)
            _, _, _, inner_id, inner_seq = struct.unpack("!BBHHH", trReplyPacket[ip_header_len + 8 + ip_header_inner_len: ip_header_len + 8 + ip_header_inner_len + 8])


        return inner_id, inner_seq, icmpType
...
class MultiThreadedTraceRoute(Traceroute):
    ...
    def __init__(self, args):
        ...
        # ---- print results grouped by TTL
        # group probe keys by TTL
        ttl_groups = {ttl: [] for ttl in range(1, MAX_TTL + 1)}
        for key in self.pkt_keys:
            if key in self.sent_times:
                ttl, _ = self.sent_times[key]
                ttl_groups[ttl].append(key)

        # print each TTL on its own line
        for ttl in range(1, MAX_TTL + 1):
            keys = ttl_groups[ttl]
            if len(keys) == 0:
                continue  # nothing was sent for this TTL

            hop_addrs_for_ttl = {k: self.hop_addrs.get(k) for k in keys}
            rtts_for_ttl = {k: self.rtts.get(k) for k in keys}

            if hop_addrs_for_ttl[keys[0]] != socket.gethostbyname(args.hostname):
                try:
                    self.printMultipleResults(ttl, keys, hop_addrs_for_ttl, rtts_for_ttl, args.hostname)
                except:
                    print(str(ttl) + '   * * *')
            else:
                try:
        ...
        while ttl <= MAX_TTL:
            ...
            for _ in range(3):  
                if args.protocol == "icmp":
                    ...
                    seq_num = random.randint(1, 65535)
                    timeSent = self.sendOnePing(self.dstAddress, packet_id, seq_num, ttl, 52)

                    with self.lock:
                        self.pkt_keys.append(seq_num)
                        self.sent_times[seq_num] = (ttl, timeSent)


                elif args.protocol == "udp":
                    dstPort += 1
                    timeSent = self.sendOneUdpProbe(self.dstAddress, dstPort, ttl, 52)

                    with self.lock:
                        self.pkt_keys.append(dstPort)
                        self.sent_times[dstPort] = (ttl, timeSent)
    ...
    def receive_responses(self):
        ...
        while not self.send_complete.is_set():
            ...
            if args.protocol == "icmp":
                ...
                with self.lock:
                    if inner_seq in self.sent_times:
                        ttl, timeSent = self.sent_times[inner_seq]
                        rtt = timeRecvd - timeSent
                        self.rtts[inner_seq] = rtt
                        self.hop_addrs[inner_seq] = hopAddr

                        # Check if destination reached (Echo Reply)
                        if icmpType_outer == 0:
                            self.isDestinationReached = True
                ...
                with self.lock:
                    if dstPortReceived in self.sent_times:
                        ttl, timeSent = self.sent_times[dstPortReceived]
                        rtt = timeRecvd - timeSent
                        self.rtts[dstPortReceived] = rtt
                        self.hop_addrs[dstPortReceived] = hopAddr

                        # Destination reached if ICMP type is 3 (Port Unreachable)
                        if icmpType == 3:
                            self.isDestinationReached = True
...
class Proxy(NetworkApplication):
    ...
    def __init__(self, args):
        ...
        while True:
            ...
            # 5. Create a new thread to handle each client request
            threading.Thread(target=self.handlerequest,
                             args=(connectionSocket,)).start()

        serverSocket.close()

    def sendrequesttoserver(self, request_bytes, host, port=80):
        try:
            remoteSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            remoteSocket.connect((host, port))
            remoteSocket.sendall(request_bytes)
            return remoteSocket
        except Exception as e:
            print("Proxy: cannot connect to server:", e)
            return None

    def receiveserverresponse(self, remotesocket):
        data = b""
        try:
            while True:
                chunk = remotesocket.recv(MAX_DATA_RECV)
                if not chunk:
                    break
                data += chunk
        except:
    ...
    def sendresponsetoclient(self, connectionsocket, data):
        try:
            connectionsocket.sendall(data)
        except Exception as e:
            print("Proxy: error sending to client:", e)
    ...
    def handlerequest(self, connectionsocket):
        try:
            ...
            request_bytes = connectionsocket.recv(MAX_DATA_RECV)
            ...
            if method.upper() != "GET":
                # Only handle GET
                connectionsocket.send(b"HTTP/1.1 405 Method Not Allowed\r\n\r\n")
                connectionsocket.close()
                return
            ...
            if host is None:
                print("Proxy: No Host header or absolute URI host.")
                connectionsocket.close()
                return

            cache_key = host + path
            cache_file = self._cache_filename(cache_key)

            # Check cache
            if cache_key in self.cache and os.path.exists(cache_file):
                print("Proxy: Cache HIT ->", cache_key)
                with open(cache_file, "rb") as f:
                    data = f.read()
                self.sendresponsetoclient(connectionsocket, data)
                connectionsocket.close()
                return

            print("Proxy: Cache MISS ->", cache_key)

            # 3. Forward client request to server
            remotesocket = self.sendrequesttoserver(request_bytes, host)
            if remotesocket is None:
                connectionsocket.close()
                return

            # 4. Receive server response
            data = self.receiveserverresponse(remotesocket)
            remotesocket.close()

            # 5. Cache response on disk
            try:
                with open(cache_file, "wb") as f:
                    f.write(data)
                self.cache[cache_key] = cache_file
            except Exception as e:
                print("Proxy: error writing cache file:", e)

            # 6. Send response back to client
            self.sendresponsetoclient(connectionsocket, data)

        except Exception as e:
            print("Proxy: error handling client request:", e)
